...
Blueprints.prototype.generate = function(type, overrideProperties, callback) {
    ...
    async.forEach(
      ...
      function(key, callback) {
        ...
        if (typeof property == 'function') {
          Blueprints.get(properties, key, callback)
        } else {
          callback()
        }
      },
...
Blueprints.prototype.blueprint = function(type, definition) {
  if (definition) {
...
Blueprints.evaluateProperty = function(context, property, count, callback) {
  var functions = []
  while(count--) {
...
Blueprints.prototype.evaluateProperty = function(property, count, callback) {
  Blueprints.evaluateProperty(this, property, count, callback)
}

Blueprints.prototype.linkProperties = function(blueprint, properties, callback) {
  ...
  async.reduce(
    ...
    function(memo, key, callback) {
      ...
      if (typeof item == 'function' && typeof memo[key] == 'number') {
        Blueprints.evaluateProperty(memo, item, memo[key], function(err, results) {
          memo[key] = results
          callback(null, memo)
        })
      } else {
...
Blueprints.prototype.randomBetween = function(min, max) {
  if (max < min) return 0
  return Math.floor(Math.random() * (max - min + 1)) + min
}

Blueprints.generate = function() {
  throw new Error('Not implemented')
}

Blueprints.prototype.generator = function property(func) {
  func.isGenerator = true
  return func
}

Blueprints.get = function(object, property, callback) {
...
Blueprints.prototype.get = function(property, callback) {
  Blueprints.get(this, property, callback)
}
